...
public sealed record AuthMetadata
{
    ...
    public AuthMetadata(bool isPublic = false, bool apiClientPermitted = false, string? domain = null, string? scope = null, string? permission = null)
    {
        IsPublic = isPublic;
    ...
    public bool HasAccess(UserPermits? permits, ClientApi? clientApi)
    {
        if (IsPublic)
            return true;

        if(ApiClientPermitted && clientApi is not null)
            return true;
    ...
    public AuthMetadata NewWith(string? domain = null, string? scope = null, string? permission = null)
    {
        ...
        return new AuthMetadata(IsPublic, ApiClientPermitted, domain, scope, permission);
    }
}
